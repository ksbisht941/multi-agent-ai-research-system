...
def chat_node(state: ChatState) -> dict:
    ...
    logger.info(
        f"Trimmed history to {count_tokens_approximately(trimmed_history)} tokens (budget: {max_tokens})"
    )
...
def human_approval_node(state: ChatState) -> Command:
    ...
    logger.info(
        "LangGraph interrupt triggered: Awaiting Human approval for Search query."
    )
        ...
        return Command(
            ...
            update={
                "messages": [
                    ...
                    AIMessage(
                        content="❌ Web search was rejected by the human reviewer."
                    ),
                ]
            },
        )
...
def get_chatbot(force_recompile: bool = False):
    ...
    if _compiled_chatbot is None or force_recompile:
        ...
        workflow.add_conditional_edges(
            "chat_node", route_tools, ["human_approval", "tools", END]
        )
