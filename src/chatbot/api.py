...
def list_threads():
    ...
    try:
        ...
        logger.error("Error fetching threads from DB: %s", e, exc_info=True)
        return {"threads": []}
    finally:
        conn.close()
...
def download_pdf(filename: str):
    ...
    return FileResponse(
        path=str(pdf_path), filename=filename, media_type="application/pdf"
    )
...
async def resolve_approval(req: ApprovalRequest):
    ...
    async def resume_generator():
        try:
            ...
            def sync_resume():
                return chatbot.stream(
                    resume_command, config=config, stream_mode=["messages", "updates"]
                )
